export type {
  SslDetailDokumentuCjDokumentu,
  SslDetailDokumentuDoruceni,
  SslDetailDokumentuEDoruceni,
  SslDetailDokumentuHistorieDokumentuItem,
  SslDetailDokumentuPrilohyDokumentuItem,
  SslDetailDokumentuRequest,
  SslDetailDokumentuResponse,
  SslDetailDokumentuSouvisejiciDokumentyItem,
  SslDetailDokumentuSpisovnaItem,
  SslDetailDokumentuSslDokument,
  SslDetailDokumentuSslObsahSpisItem,
  SslDetailDokumentuSslSpis,
  SslDetailDokumentuVlozenoDoSpisu,
  SslDetailDokumentuWflDokument,
} from './Detail-dokumentu'
export type {
  SslPrehledDokumentuPrehledDokumentuItem,
  SslPrehledDokumentuRequest,
  SslPrehledDokumentuRequestRizeniPrehledu,
  SslPrehledDokumentuResponse,
  SslPrehledDokumentuStavPrehledu,
} from './Prehled-dokumentu'
export type {
  SslPridatSouborPridatSoubor,
  SslPridatSouborRequest,
  SslPridatSouborResponse,
} from './Pridat-soubor'
export type { SslPrideleniPrideleni, SslPrideleniRequest, SslPrideleniResponse } from './Prideleni'
